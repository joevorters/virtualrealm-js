...
Session.prototype = {
  ...
  readCommand: function() {
    ...
    if (!self.isWebSocket) {
      ...
      this._socket.on('message', receiveHeaders);
    }
    this.read(12);
    function next() {
...
SessionPool.prototype = {
  ...
  forEach: function forEach(cb, thisArg) {
    return _.forOwn(this.getSessions(), cb, thisArg);
  },
  add: function add(socket, opts) {
    let id = uuid.v1();
    let session = this.getSessions()[id] = Session(socket, this);
    session.isWebSocket = Boolean(opts.isWebSocket);
    session._socket.pause();
    session.id = id;
    session._pool = this;
    session.rpc = RPC(session);
    if (session.isWebSocket) session.readCommand();
    else session._socket.on('readable', function() {
      session.readCommand();
    });
  },
  get: function get(id) {
