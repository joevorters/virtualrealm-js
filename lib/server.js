...
Server.prototype = {
  ...
  listen: function() {
    this.getHttpServer().listen(this.getConfig().httpPort, () => {
      log.debug(`${chalk.green('HTTPS Server')} ${chalk.yellow('bound')} on port ${chalk.red(String(this.getConfig().httpPort))}`);
    });
    ...
    let self = this;
    if (this.getRedirectServer()) {
      this.getRedirectServer().listen(this.getConfig().redirect, function() {
        log.debug(`${chalk.green('HTTP Server')} ${chalk.yellow('bound')} on port ${chalk.red(String(self.getConfig().redirect))}`);
      });
    }
  }
};
...
function Server(config) {
  ...
  if (config.redirect) {
    ...
    this._redirectServer.use('*', function(req, res, next) {
      console.log(req);
    });
  }
  this._config = config;
